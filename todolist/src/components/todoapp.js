...
const Todoapp = () => {
  ...
  async function AddTodo(event) {
    ...
    if (!userInput.trim()) {
      ...
      await msgUpdate({ variables, refetchQueries: [{ query: GET_MSG_LIST }] });
      setUserInput("");
      setIsEditItem(null);
      setEditButton(false);
    } else {
      ...
      await msgAdd({ variables, refetchQueries: [{ query: GET_MSG_LIST }] });
      setUserInput("");
    }
  }
  ...
  async function deleteTodo(index) {
    ...
    if (isConfirmed) {
      ...
      await msgDelete({ variables, refetchQueries: [{ query: GET_MSG_LIST }] });
    }
  }
      ...
      <div className="main-div">
        <div className="child-div">
          <figure>
            <figcaption>
              <a href="index.html">
                Todo List <i className="fa-solid fa-list-check" />
              </a>
            </figcaption>
          </figure>
            ...
            {data
              ? data.msglist.map((items) => (
                  <div
                    className="eachItem"
                    key={items.id}
                    onDoubleClick={() => toggleTodo(items)}
                  >
                    <i className="far fa-check-square add-btn" />
                    <h3 className={`${items.toggle && "strike"}`}>
                      {items.jobtodo}
                    </h3>
          ...
          <span className="footer">
            Made with <span>&#10084;</span> by Ancod3r
          </span>
        </div>
      </div>
    </>
